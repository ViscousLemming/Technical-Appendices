...
def pack_bool_tensor(x: "torch.Tensor") -> int:
    ...
    if torch is None or np is None:
        raise ImportError("pack_bool_tensor requires torch and numpy")
...
def unpack_bool_tensor(bitset: int, n_bits: int, *, device: str = "cpu") -> "torch.Tensor":
    """Unpack an integer bitset into a 1D boolean tensor of length n_bits.

    Requires torch and numpy.
    """
    if torch is None or np is None:
        raise ImportError("unpack_bool_tensor requires torch and numpy")
    if n_bits < 0:
        raise ValueError("n_bits must be non negative")
    if bitset < 0:
        raise ValueError("bitset must be non negative")
