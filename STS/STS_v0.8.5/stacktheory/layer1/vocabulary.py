...
class Vocabulary:
    ...
    # ------------------------------------------------------------------
    # Language size estimation and enumeration
    # ------------------------------------------------------------------

    #: Default ceiling for full materialisation of L_v.
    #: This is a soft safety limit, not a theoretical one.
    #: Callers can raise it via the *max_language_size* parameter.
    DEFAULT_MAX_LANGUAGE_SIZE: int = 2_000_000

    def count_language(self) -> int:
        """Return |L_v| without materialising any masks.

        Uses the same pruned recursion as the enumerator but only counts.
        The cost is O(|L_v|) time and O(|v|) stack space.
        """
        m = self.size
        prog_bits = self._prog_bits
        full = self.env.all_states_bitset()

        def rec(start: int, bits: int) -> int:
            total = 1          # count the current statement
            for j in range(start, m):
                new_bits = bits & prog_bits[j]
                if new_bits != 0:
                    total += rec(j + 1, new_bits)
            return total

        return rec(0, full)

    def iter_language_masks(self) -> Iterator[int]:
        """Yield every mask in L_v one at a time.

        This is a streaming alternative to induced_language_masks.
        It uses O(|v|) stack space regardless of |L_v|.
        Truth set caching is populated as a side effect.
        """
        m = self.size
        prog_bits = self._prog_bits
        full = self.env.all_states_bitset()

        def rec(start: int, mask: int, bits: int) -> Iterator[int]:
            yield mask
            self._truth_cache[mask] = Program(self.env, bits)
            for j in range(start, m):
                new_bits = bits & prog_bits[j]
                if new_bits != 0:
                    yield from rec(j + 1, mask | (1 << j), new_bits)

        yield from rec(0, 0, full)

    def induced_language_masks(self, *, max_language_size: Optional[int] = None) -> List[int]:
        """Enumerate L_v as a list of masks.

        The cost is O(|L_v|) time and memory.  A safety check prevents
        accidental blowup: if |L_v| exceeds *max_language_size*
        (default ``DEFAULT_MAX_LANGUAGE_SIZE``), a ``ValueError`` is raised
        before any allocation.

        For vocabularies with heavy mutual exclusivity (many contradictory
        program pairs), |L_v| can be orders of magnitude smaller than 2^|v|.
        Use ``count_language`` to check feasibility first.
        """
        if self._L_masks_cache is not None:
            return list(self._L_masks_cache)

        if max_language_size is None:
            max_language_size = self.DEFAULT_MAX_LANGUAGE_SIZE

        lv_size = self.count_language()
        if lv_size > max_language_size:
            raise ValueError(
                f"|L_v| = {lv_size:,} exceeds max_language_size = {max_language_size:,} "
                f"for |v| = {self.size}.  Use count_language() to inspect, "
                f"iter_language_masks() to stream, or raise the limit."
            )
        ...
        def rec(start: int, mask: int, bits: int) -> None:
            out.append(mask)
            self._truth_cache[mask] = Program(self.env, bits)
            for j in range(start, self.size):
                new_bits = bits & prog_bits[j]
                if new_bits != 0:
